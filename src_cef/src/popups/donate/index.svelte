<script>
    ...
    const onDonate = () => {
        ...
        if (!selectType) {
            ...
            window.notificationAdd(4, 9, `Вы начали процесс покупки Battle Pass. Совершите донат через личный кабинет (lk.flycityrp.com) в размере 1999 руб, и премиум-доступ автоматически станет доступен.`, 25000);
            executeClient ("client.donatepack.donate", popupData.id);
            executeClient ("client.donatepack.close");

            /*axios.post('https://pay.flycityrp.com/', qs.stringify({
                name: $accountLogin,
    ...
    <div class="donatepopup__payments">
        <div class="box-between">
            <div class="box-column">
                ...
                <div class="donatepopup__small">{!selectType ? "Купить за FlyCoins" : "Купить за рубли"}</div>
        ...
        {#if selectType === 1}
            {translateText('popups', 'После нажатия кнопки "Подтвердить" у вас есть 5 минут, чтобы совершить платёж.')}<br/><br/>{translateText('popups', 'Если выйти из игры до завершения транзакции, то на аккаунт будут зачислены FlyCoins по текущему курсу.')}
        {/if}
        </div>
    </div>
